...
%% Loading
% Path to .mhd file
mhd_file ="D:\Downloads\EEE 312 DSP lab\LUNA16_Small\1.3.6.1.4.1.14519.5.2.1.6279.6001.108197895896446896160048741492.mhd";
...
opts = detectImportOptions("D:\Downloads\EEE 312 DSP lab\LUNA16_Small\annotations.csv");
opts = setvartype(opts, {'coordX','coordY','coordZ','diameter_mm'}, 'double');
data = readtable("D:\Downloads\EEE 312 DSP lab\LUNA16_Small\annotations.csv", opts);
...
%% Denoising
denoised = medfilt2(norm_slice, [3 3]);  % built-in median filter

% Display
figure('Name', '✅ Denoised CT Slice');
imshow(denoised);
colormap gray;
...
adhist_eq = adapthisteq(denoised);
...
% Apply manual CLAHE
enhanced = manual_adapthisteq(denoised, 64, 0.01);
...
figure;
subplot(1,2,1);
histogram(denoised(:), 100);  % Before AHE
title('Before AHE');
xlabel('Pixel Value');
...
% Histogram of original normalized image
figure;
histogram(norm_slice(:), 100);
...
%% Lung Masking – Equivalent of kmader’s Python logic
 ...
 %Output: lung_mask, masked_lung_img
...
binary_closed = imclose(binary_opened, strel('disk', 3));

% Step 5: Keep top 2 largest connected components (lungs)
cc = bwconncomp(binary_closed);
stats = regionprops(cc, 'Area');
[~, idx] = sort([stats.Area], 'descend');
lung_mask = false(size(binary_closed));
for i = 1:min(2, numel(idx))
    lung_mask(cc.PixelIdxList{idx(i)}) = true;
end

% Step 6: Final mask cleanup
lung_mask = imclose(lung_mask, strel('disk', 5));
lung_mask = imfill(lung_mask, 'holes');

% Step 7: Apply the lung mask to the enhanced image
masked_lung_img = enhanced;
masked_lung_img(~lung_mask) = 0;

% Visualization of Lung Masking Results
figure;
...
edges_binary = bwareaopen(edges_binary, Ap2min);  % remove tiny blobs
subplot(2,1,1); imshow(edges_binary); title('areaopen');
edges_binary = imclose(edges_binary, strel('disk', 3));
...
for k = 1:length(stats)
    ...
    round_ok = stats(k).Eccentricity < 0.8 && stats(k).Solidity > 0.3;

    area = stats(k).FilledArea;
    perimeter = stats(k).Perimeter;
    % Compute roundness using FilledArea
    roundness = (4 * pi * area) / (perimeter ^ 2);
    round2_ok = roundness > 0.75;  % Adjust threshold as needed

    
    hollowness_ok = stats(k).EulerNumber == 0;
    horatio_ok = stats(k).Area / stats(k).FilledArea < 0.8;
...
if gt_detected
    ...
    fprintf('Detected Diameter: %.2f mm (Expected: %.2f mm)\n', ...
